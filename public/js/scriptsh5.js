...
function myFunc( data ){
  ...
  $.each(data.media_list, function(i,item){
        ...
        if(i < 5){
        	//into the body of the page
            $("#items").append('<div id="video-item"><div class="img-container"><img src="' + item.thumbnails[0].url + '"></div><p class="vidtitle">' + item.title + '<!--' + item.publish_date + '--></p><div id="playbutton"><div class="div btn"><a href="videopageh5?mediaid='+item.media_id+'">Play</a></div></div> <div class="clear"></div>');
            // $("#title").append("<p>" + item.title + "</p><p>" + item.media_id + "</p><p>" + item.description + "</p><p>" + item.thumbnails[0].url + "</p>");
        }
	});
}
